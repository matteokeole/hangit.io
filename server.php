<?php
	...
	class GameServer {
		...
		public function get_foundIndex($game_url) {
			...
			if (is_null($value["foundIndex"])) $value["foundIndex"] = 0;
			else $value["foundIndex"] += 1;
		...
		// When one row in `player` is deleted, this removes all tables containing the id_player attribute
		...
		// When one row in `game` is deleted, this removes all tables containing the id_game attribute
		...
		/*public function set_put_player_in_round($game_url) {
			$setplayerinround = $this->bdd->prepare("INSERT INTO `round_player` (id_player, id_round) VALUES ((SELECT id_player FROM `player` WHERE id_game = ?), (SELECT id_round FROM `round` WHERE id_game = ?)");
			$setplayerinround->execute(array($game_url, $game_url));
		}*/
	}
	// Game server
	$Partie = new GameServer();
	if (isset($_POST["link_game"])) {
		$Link_game = htmlspecialchars($_POST["link_game"]);
		if ($Partie->url_existe($Link_game)) echo "Error: the link already exists!";
		else {
	...
	if (isset($_POST["url"], $_POST["nickname"], $_POST["color"], $_POST["roundPlayer"])) {
		...
		do {
			...
			foreach ($players as $player) {
				if ($player["nickname"] == $nickname) $found = true;
			}
			if ($found) {
				if (substr($nickname, - strlen((string) $i)) == $i){
					$nickname = $base . " #" . $i;
					$i++;
				} else $nickname = $base . " #" . $i;
			}
		} while ($found);
	...
	if (isset($_GET["get_foundIndex"])) echo $Partie->get_foundIndex($_GET["get_foundIndex"]);
